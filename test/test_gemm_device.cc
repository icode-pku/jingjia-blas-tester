...
void test_gemm_device_work( Params& params, bool run )
{
    ...
    if(testcase == 0){
        ...
        blas::gemm( Layout::ColMajor, Op::NoTrans, Op::NoTrans,   m, n, k, alpha, dA, lda, dB, k-1, beta, dC, ldc, queue, testcase, error_name );
        Blas_Match_Call( result_match(error_name, "CUBLAS_STATUS_INVALID_VALUE", all_testcase, passed_testcase, failed_testcase), error_name);
        //case 13: Test the return value when col-major order, tansB=Trans, ldb is an illegal value
        blas::gemm( Layout::ColMajor, Op::NoTrans, Op::Trans,     m, n, k, alpha, dA, lda, dB, n-1, beta, dC, ldc, queue, testcase, error_name );
        Blas_Match_Call( result_match(error_name, "CUBLAS_STATUS_INVALID_VALUE", all_testcase, passed_testcase, failed_testcase), error_name);
        //case 14: Test the return value when col-major order, tansB=ConjTrans, ldb is an illegal value
        blas::gemm( Layout::ColMajor, Op::NoTrans, Op::ConjTrans, m, n, k, alpha, dA, lda, dB, n-1, beta, dC, ldc, queue, testcase, error_name );
        Blas_Match_Call( result_match(error_name, "CUBLAS_STATUS_INVALID_VALUE", all_testcase, passed_testcase, failed_testcase), error_name);

        //case 15: Test the return value when row-major order, tansB=NoTrans, ldb is an illegal value
        blas::gemm( Layout::RowMajor, Op::NoTrans, Op::NoTrans,   m, n, k, alpha, dA, lda, dB, n-1, beta, dC, ldc, queue, testcase, error_name );
        Blas_Match_Call( result_match(error_name, "CUBLAS_STATUS_INVALID_VALUE", all_testcase, passed_testcase, failed_testcase), error_name);
        //case 16: Test the return value when row-major order, tansB=Trans, ldb is an illegal value
        blas::gemm( Layout::RowMajor, Op::NoTrans, Op::Trans,     m, n, k, alpha, dA, lda, dB, k-1, beta, dC, ldc, queue, testcase, error_name );
        Blas_Match_Call( result_match(error_name, "CUBLAS_STATUS_INVALID_VALUE", all_testcase, passed_testcase, failed_testcase), error_name);
        //case 17: Test the return value when row-major order, tansB=ConjTrans, ldb is an illegal value
        blas::gemm( Layout::RowMajor, Op::NoTrans, Op::ConjTrans, m, n, k, alpha, dA, lda, dB, k-1, beta, dC, ldc, queue, testcase, error_name );
        Blas_Match_Call( result_match(error_name, "CUBLAS_STATUS_INVALID_VALUE", all_testcase, passed_testcase, failed_testcase), error_name);
...
//-------------start half datatype----------------------------------------------
template <>
void test_gemm_device_work<Half_t, Half_t, Half_t>( Params& params, bool run )
{
    // printf("half gemm...\n");
    using namespace testsweeper;
    using std::real;
    ...
    half* hA    = new half[ size_A ];
    half* hB    = new half[ size_B ];
    half* hC    = new half[ size_C ];
    // device specifics
    blas::Queue queue( device );
    half* dA;
    half* dB;
    ...
    dA = blas::device_malloc<half>( size_A, queue );
    dB = blas::device_malloc<half>( size_B, queue );
    dC = blas::device_malloc<half>( size_C, queue );
    ...
    blas::copydatafloat2float(size_A, A, A);
    blas::copydatafloat2float(size_B, B, B);
    blas::copydatafloat2float(size_C, C, C);
    lapack_lacpy( "g", Cm, Cn, C, ldc, Cref, ldc );


    //from float to half
    blas::copydatafloat2half(size_A, A, hA);
    blas::copydatafloat2half(size_B, B, hB);
    blas::copydatafloat2half(size_C, C, hC);

    blas::device_copy_matrix(Am, An, hA, lda, dA, lda, queue);
    blas::device_copy_matrix(Bm, Bn, hB, ldb, dB, ldb, queue);
    blas::device_copy_matrix(Cm, Cn, hC, ldc, dC, ldc, queue);
    queue.sync();

    // norms for error check
    float work[1];
    float Anorm = lapack_lange( "f", Am, An, A, lda, work );
    float Bnorm = lapack_lange( "f", Bm, Bn, B, ldb, work );
    float Cnorm = lapack_lange( "f", Cm, Cn, C, ldc, work );
    ...
    if(testcase == 0){
        ...
        double gflop = blas::Gflop< float >::gemm( m, n, k );
        blas::device_copy_matrix(Cm, Cn, dC, ldc, hC, ldc, queue);
        queue.sync();
        
        //convert half to float
        blas::copydatahalf2float(size_C, hC, C);
        ...
        if (params.ref() == 'y' || params.check() == 'y') {
            ...
            if(params.iscorrect()==1){
                ...
                bool okay;
                check_gemm( Cm, Cn, k, alpha, beta, Anorm, Bnorm, Cnorm,
                            Cref, ldc, C, ldc, verbose, &error, &okay, true);
                params.error() = error;
                params.okay() = okay;
            }
        }
...
void test_gemm_device( Params& params, bool run )
{
    switch (params.datatype()) {
        ...
        case testsweeper::DataType::Half:
            test_gemm_device_work< Half_t, Half_t, Half_t >( params, run );
            break;
