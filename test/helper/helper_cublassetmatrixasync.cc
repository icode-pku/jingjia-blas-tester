...
{
    TestId CaseId(9, std::string("cublasSetMatrixAsync(int rows, int cols, int elemSize, const void *A, int lda, void *B, int ldb, cudaStream_t stream)"));
    ...
    for(int aid=0; aid<2; aid++){
        for(int bid=0; bid<2; bid++){
            ...
            stat = cublasSetMatrixAsync(rows, cols, elemSize, A, lda[aid], dA, ldb[bid], stream);
    ...
    stat = cublasSetMatrixAsync(-1, cols, elemSize, A, lda[0], dA, ldb[0], stream);
    ...
    stat = cublasSetMatrixAsync(rows, -1, elemSize, A, lda[0], dA, ldb[0], stream);
    HelperTestCall("cublasSetMatrixAsync", check_return_status(stat, "CUBLAS_STATUS_INVALID_VALUE", All_tests, Passed_tests, Failed_tests), stat, "CUBLAS_STATUS_INVALID_VALUE");
    HelperSafeCall(cudaStreamSynchronize(stream));

    //test case 7: Testing for illegal parameter elemSize
    CaseId.TestProblemHeader(2, false, "-1");
    stat = cublasSetMatrixAsync(rows, cols, -1, A, lda[0], dA, ldb[0], stream);
    HelperTestCall("cublasSetMatrixAsync", check_return_status(stat, "CUBLAS_STATUS_INVALID_VALUE", All_tests, Passed_tests, Failed_tests), stat, "CUBLAS_STATUS_INVALID_VALUE");
    HelperSafeCall(cudaStreamSynchronize(stream));

    //test case 8: Testing for illegal parameter lda
    CaseId.TestProblemHeader(4, false, "-1");
    stat = cublasSetMatrixAsync(rows, cols, elemSize, A, -1, dA, ldb[0], stream);
    HelperTestCall("cublasSetMatrixAsync", check_return_status(stat, "CUBLAS_STATUS_INVALID_VALUE", All_tests, Passed_tests, Failed_tests), stat, "CUBLAS_STATUS_INVALID_VALUE");
    HelperSafeCall(cudaStreamSynchronize(stream));

    //test case 9: Testing for illegal parameter ldb
    CaseId.TestProblemHeader(6, false, "-1");
    stat = cublasSetMatrixAsync(rows, cols, elemSize, A, lda[0], dA, -1, stream);
    HelperTestCall("cublasSetMatrixAsync", check_return_status(stat, "CUBLAS_STATUS_INVALID_VALUE", All_tests, Passed_tests, Failed_tests), stat, "CUBLAS_STATUS_INVALID_VALUE");
    HelperSafeCall(cudaStreamSynchronize(stream));
