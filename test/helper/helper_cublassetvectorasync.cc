...
{
    TestId CaseId(7, std::string("cublasSetVectorAsync(int n, int elemSize, const void *x, int incx, void *y, int incy, cudaStream_t stream)"));
    ...
    for(int i=0; i<2; i++){
        for(int j=0; j<2;j++){
            ...
            stat = cublasSetVectorAsync(n, sizeof(float), x, incx[i], dx, incy[j], stream);
    ...
    stat = cublasSetVectorAsync(size_x, -1, x, incx[0], dx, incy[0], stream);
    ...
    stat = cublasSetVectorAsync(size_x, sizeof(float), x, -1, dx, incy[0], stream);
    HelperTestCall("cublasSetVectorAsync", check_return_status(stat, "CUBLAS_STATUS_INVALID_VALUE", All_tests, Passed_tests, Failed_tests), stat, "CUBLAS_STATUS_INVALID_VALUE");
    HelperSafeCall(cudaStreamSynchronize(stream));

    //test case 7: Testing for illegal parameter incy
    CaseId.TestProblemHeader(5, false, "-1");
    stat = cublasSetVectorAsync(size_x, sizeof(float), x, incx[0], dx, -1, stream);
    HelperTestCall("cublasSetVectorAsync", check_return_status(stat, "CUBLAS_STATUS_INVALID_VALUE", All_tests, Passed_tests, Failed_tests), stat, "CUBLAS_STATUS_INVALID_VALUE");
    HelperSafeCall(cudaStreamSynchronize(stream));
