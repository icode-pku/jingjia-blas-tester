...
void test_trsv_device_work( Params& params, bool run )
{
    ...
    if(testcase == 0){
        ...
        blas::trsv( layout,    Uplo(0), trans, diag,     n, dA, lda, dx, incx, queue, testcase, error_name );
        ...
        blas::trsv( layout, uplo, Op(0), diag, n, dA, lda, dx, incx, queue, testcase, error_name );
        Blas_Match_Call( result_match(error_name, "CUBLAS_STATUS_INVALID_VALUE", all_testcase, passed_testcase, failed_testcase), error_name);
        //case 3: Test diag is an illegal value
        blas::trsv( layout, uplo, trans, Diag(0), n, dA, lda, dx, incx, queue, testcase, error_name );
        Blas_Match_Call( result_match(error_name, "CUBLAS_STATUS_INVALID_VALUE", all_testcase, passed_testcase, failed_testcase), error_name);
        //case 4: Test the return value when n is an illegal value
        blas::trsv( layout, uplo, trans, diag, -1, dA, lda, dx, incx, queue, testcase, error_name );
        Blas_Match_Call( result_match(error_name, "CUBLAS_STATUS_INVALID_VALUE", all_testcase, passed_testcase, failed_testcase), error_name);
        
        //case 5: Test the return value when lda is an illegal value and trans is NoTrans
        blas::trsv( Layout::ColMajor, uplo, Op::NoTrans, diag, n, dA, n-1, dx, incx, queue, testcase, error_name );
        Blas_Match_Call( result_match(error_name, "CUBLAS_STATUS_INVALID_VALUE", all_testcase, passed_testcase, failed_testcase), error_name);
        //case 6: Test the return value when lda is an illegal value and trans is Trans
        blas::trsv( Layout::ColMajor, uplo, Op::Trans, diag, n, dA, n-1, dx, incx, queue, testcase, error_name );
        Blas_Match_Call( result_match(error_name, "CUBLAS_STATUS_INVALID_VALUE", all_testcase, passed_testcase, failed_testcase), error_name);
